...
uintptr_t esp_dlmap(void *handle, uintptr_t address)
{
    if (!handle || !dlmod_validate_handle(handle)) {
        dlerror_set("Invalid module handle");
        return 0;
    }
    ...
    struct dlmod_slist_t *mod = (struct dlmod_slist_t *)handle;
    if (!mod->elf) {
        dlerror_set("Module has no ELF image");
        return 0;
    }

    const uintptr_t result = esp_elf_map_sym(mod->elf, address);
    dlerror_set(result == 0 ? "Virtual address is outside the module image" : NULL);
    return result;
}
