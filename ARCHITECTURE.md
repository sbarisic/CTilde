...
`Examples.sln` is a discovery surface, not a cross-target build graph. Its 18 C~ projects are grouped as language/hosted programs, systems targets, managed modules, and T-CAN variants; none has a `Build.0` mapping. The [example catalog](examples/README.md) records each project's toolchain or hardware prerequisite. Focused hosted examples execute common features without optional hardware, while target examples retain their own explicit runners.
