...
Use the device's current address and a known-hosts file containing its verified host key. The probe does not request or store passphrases. It checks a remote `free` command and a uniquely named SFTP upload/download, compares hashes, then removes its remote file. Results and timeout details are written to `ssh-acceptance.json`. These checks do not establish interactive-session, rekey, maximum-packet, or stack acceptance.
...
Start the service as `sshd.ctm &`. Its mailbox accepts `status`, `reload-keys`, `disconnect`, and `stop`. The configured shell path is fixed to `/storage/modules/shell.ctm`, bypassing SD module precedence for the administration shell. Reload constructs a complete replacement key set; an active connection keeps its captured keys until it disconnects. OpenSSH public-key authentication now passes on the ESP32. Remote commands and SFTP failed after authentication in the shared-services build. The incremental SFTP framer has passed host checks. Full interoperability, malformed-packet fuzzing, endurance, and independent security review remain pending. See [current validation](DRAFT051_ABI23_VALIDATION.json) for the tested scope. Do not treat it as production-ready or expose it to an untrusted network.
