...
An ESP-IDF manifest can select `espIdf.artifact: "managed-module"` to emit a Runtime ABI 23 / Managed Module ABI 4 `.ctm` image and deterministic schema-3 `.ctmeta.json` reference. This artifact requires `build.cLayout: "modules"` and a `managedModule` block. Managed module names accept at most 63 ASCII bytes and versions at most 31 ASCII bytes. Optional exact `managedModule.nativeSources` must name checked-in `.c` files declared by the module's ESP-IDF `main` component. Those sources and their project-local quoted-header closure enter the build identity without changing the managed API hash. Xtensa modules may place selected method bodies in named process-local overlays while keeping stable call stubs resident. The hosted target does not load `.ctm` files; hosted dynamic loading uses the separate native C ABI `[NativeImport]` facility.
...
Hosted and freestanding native object-cache format 2 resolves transitive headers with the selected compiler before reuse. Old-format entries are ignored; unavailable dependency information produces an uncached build. The native object-cache format is independent of the language and runtime ABI versions. Explicit `restore` and `update` commands support Ctrl+C and preserve the last valid lockfile on failure or cancellation.
