...
internal sealed partial class CEmitter : ILoweringServices
{
    ...
    private ImmutableArray<IrFunction> _irFunctions = ImmutableArray<IrFunction>.Empty;
    ...
    private bool UsesEspRuntimeIo => IsManagedModule || IsEspIdf && Model.RuntimeImplementations.Keys.Any(role => role == RuntimeImplementationRole.PathSeparator || role is >= RuntimeImplementationRole.FileOpen and <= RuntimeImplementationRole.CurrentDirectorySet);
    ...
    {
        _irFunctions = program.Functions;
        ...
        string debugMap)
    {
        ...
        var sources = artifacts.Where(artifact => artifact.Kind is GeneratedCArtifactKind.RuntimeSource or GeneratedCArtifactKind.NamespaceSource or GeneratedCArtifactKind.EntrySource)
            ...
            .ToArray();
        var cmake = BuildCMakeFragment(sources, _managedModule, _managedModuleMetadata?.HasOverlays == true);
        artifacts.Add(new GeneratedCArtifact("ctilde_sources.cmake", cmake, GeneratedCArtifactKind.CMakeFragment));
        return artifacts.OrderBy(artifact => artifact.RelativePath, StringComparer.Ordinal).ToImmutableArray();
    }

    internal static string BuildCMakeFragment(IEnumerable<string> generatedSources, ManagedModuleConfiguration? managedModule, bool hasOverlays = false)
    {
        ...
        if (managedModule is { NativeSources.IsDefaultOrEmpty: false, NativeComponentDirectory: { } componentDirectory })
        {
            foreach (var source in managedModule.NativeSources.OrderBy(path => path, StringComparer.Ordinal))
            {
                var relative = Path.GetRelativePath(componentDirectory, source).Replace('\\', '/');
                cmake.Append("    \"${COMPONENT_DIR}/").Append(relative).Append("\"\n");
            }
        }
        cmake.Append(")\n");
        if (hasOverlays)
            cmake.Append("set(CTILDE_HAS_MANAGED_OVERLAYS TRUE)\n");
    ...
    {
        var overlayIds = program.Functions
            .Select(function => function.Method.OverlayName)
    ...
    private void EmitManagedModuleDescriptor(CWriter writer)
    {
        ...
        var exports = ManagedExports().ToArray();
        if (exports.Length != 0)
        {
            writer.WriteLine("static const ct_managed_export_v2 ct_managed_exports[] = {");
            foreach (var export in exports)
                writer.WriteLine($"    {{ \"{export.Identity}\", (void*)(uintptr_t)&{(export.Method.IsOverlay ? export.Method.CName : ManagedExportStubName(export.Method))} }},");
            writer.WriteLine("};");
        }
        ...
        var exportsPointer = exports.Length == 0 ? "NULL" : "ct_managed_exports";
        var importsPointer = imports.Length == 0 ? "NULL" : "ct_managed_imports";
    ...
    private sealed record ManagedImportUse(string Name, string Dependency, string Identity, MethodSymbol Method);

    private sealed record ManagedCallEntry(
        string Key,
        MethodSymbol Method,
        string StubName,
        string BodyName,
        bool ConstructorInitializer,
        uint Placement,
        uint OverlayId,
        int TargetIndex);

    private ImmutableArray<ManagedCallEntry> ManagedCallEntries()
    {
        if (!IsManagedModule)
            return [];
        var overlayIds = (_managedModuleMetadata?.Overlays ?? []).Select((item, index) => (item.Name, Id: (uint)index + 1u))
            .ToDictionary(item => item.Name, item => item.Id, StringComparer.Ordinal);
        var overlayTargetIndices = (_managedModuleMetadata?.Overlays ?? []).SelectMany(item => item.Functions)
            .ToDictionary(item => item.BodySymbol, item => item.TargetIndex, StringComparer.Ordinal);
        ...
        foreach (var function in _irFunctions)
        {
            ...
            var callable = constructorInitializer
                ...
                : function.Property is null ? method.CName : function.IsGetter ? NameMangler.Getter(function.Property) : NameMangler.Setter(function.Property);
            var body = OverlayBodyName(method, callable);
            if (!overlayTargetIndices.TryGetValue(body, out var targetIndex))
                throw new InvalidOperationException($"Overlay body '{body}' is missing from managed-module metadata.");
            entries[callable] = new ManagedCallEntry(
                $"overlay:{method.OverlayName}:{NameMangler.MethodIdentity(method)}:{callable}", method, callable, body,
                constructorInitializer, 1u, overlayIds[method.OverlayName!], targetIndex);
        }
        foreach (var export in ManagedExports())
        {
            if (export.Method.IsOverlay)
                continue;
            var stub = ManagedExportStubName(export.Method);
            entries[stub] = new ManagedCallEntry($"export:{export.Identity}", export.Method, stub, export.Method.CName,
                false, 0u, 0u, -1);
        }
        return [.. entries.Values.OrderByDescending(entry => entry.Placement)
            .ThenBy(entry => entry.Placement == 1u ? entry.TargetIndex : int.MaxValue)
            .ThenBy(entry => entry.Key, StringComparer.Ordinal)];
    }

    private static string ManagedExportStubName(MethodSymbol method) =>
        NameMangler.Artifact("ct_ms_", $"managed-export-stub:{NameMangler.MethodIdentity(method)}");
    ...
    private void EmitManagedCallStubs(CWriter writer)
    {
        ...
        for (var index = 0; index < entries.Length; index++)
        {
            ...
            var arguments = ManagedCallArguments(entry);
            var returnType = entry.ConstructorInitializer ? CType.Void : entry.Method.IsConstructor ? entry.Method.ContainingType.Type : entry.Method.ReturnType;
            ...
            if (returnType == CType.Void)
            {
                ...
                writer.WriteLine("    ct_cleanup_unwind_to(ct_call_boundary);");
                writer.WriteLine("    return;");
            }
            else
            {
                writer.WriteLine($"    {CTypeName(returnType)} ct_result = ((__typeof__(&{entry.BodyName}))(uintptr_t)ct_body_address)({arguments});");
                writer.WriteLine("    ct_cleanup_unwind_to(ct_call_boundary);");
    ...
    private static string ManagedCallArguments(ManagedCallEntry entry)
    {
        var arguments = new List<string>();
        if (entry.ConstructorInitializer || !entry.Method.IsStatic && !entry.Method.IsConstructor)
            arguments.Add("ct_self");
        foreach (var parameter in entry.Method.Parameters)
        {
            var name = NameMangler.Identifier(parameter.Name);
            arguments.AddRange(ParameterArgumentNames(parameter, name));
            if (parameter.IsSynchronousCallback)
                arguments.Add(name + "_context");
        }
        return string.Join(", ", arguments);
    }

    private IEnumerable<(string Identity, MethodSymbol Method)> ManagedExports()
    {
        if (_managedModuleMetadata is null)
            yield break;
        var declared = _managedModuleMetadata.Exports.Select(export => export.Identity).ToHashSet(StringComparer.Ordinal);
        foreach (var method in Model.ProjectTypes.SelectMany(type => type.Methods.Concat(type.Constructors))
                     .Where(method => method.Accessibility == Accessibility.Public && !method.IsEntryPoint && !method.IsNativeBoundary &&
                         !method.IsAbstract && method.ContainingType.Kind != DeclaredTypeKind.Interface && _reachableMethods.Contains(method))
                     .OrderBy(NameMangler.MethodIdentity, StringComparer.Ordinal))
        {
            ...
            if (declared.Contains(identity))
                yield return (identity, method);
        }
    }

    private IEnumerable<ManagedImportUse> ManagedImports()
    {
        if (_managedModule is null)
            yield break;
        const string prefix = "ct_managed_import_";
        foreach (var method in _externUses.Select(use => use.Method)
                     .Where(method => method.ExternName?.StartsWith(prefix, StringComparison.Ordinal) == true)
                     .DistinctBy(method => method.ExternName, StringComparer.Ordinal)
                     .OrderBy(method => method.ExternName, StringComparer.Ordinal))
        {
            var name = method.ExternName!;
            var separator = name.IndexOf('_', prefix.Length + 16);
            if (separator != prefix.Length + 16 || name.Length != separator + 1 + 64)
                throw new InvalidOperationException($"Invalid managed import symbol '{name}'.");
            var moduleHash = name.Substring(prefix.Length, 16);
            var identity = name[(separator + 1)..];
            var matches = _managedModule.References.Where(reference =>
                ManagedModuleMetadata.HashIdentity(reference.Name).StartsWith(moduleHash, StringComparison.Ordinal)).ToArray();
            if (matches.Length != 1)
                throw new InvalidOperationException($"Managed import '{name}' does not identify one exact dependency.");
            yield return new ManagedImportUse(name, matches[0].Name, identity, method);
        }
    }

    private void EmitManagedImportSupport(CWriter writer)
    {
        if (!IsManagedModule)
            return;
        foreach (var import in ManagedImports())
        {
            var method = import.Method;
            var parameters = method.Parameters.Select(parameter => CParameterDeclaration(parameter, NameMangler.Identifier(parameter.Name))).ToArray();
            var parameterTypes = method.Parameters.Select(ManagedImportParameterType).ToArray();
            ...
            writer.WriteLine($"    typedef {returnType} (*ct_import_function)({(parameterTypes.Length == 0 ? "void" : string.Join(", ", parameterTypes))});");
            writer.WriteLine($"    ct_import_function ct_target = (ct_import_function)(uintptr_t){import.Name}_address;");
            if (method.ReturnType == CType.Void)
            {
                writer.WriteLine($"    ct_target({arguments});");
            }
            else
            {
                writer.WriteLine($"    {returnType} ct_result = ct_target({arguments});");
                writer.WriteLine("    return ct_result;");
            }
            writer.WriteLine("}");
        }
    ...
    private string ManagedImportParameterType(ParameterSymbol parameter) => parameter.PassingKind switch
    {
    ...
    private void EmitManagedBinaryManifest(CWriter writer, ManagedModuleConfiguration configuration, ManagedModuleMetadata metadata)
    {
        ...
        writer.WriteLine($"    {{ UINT8_C(0x43), UINT8_C(0x54), UINT8_C(0x4D), UINT8_C(0x4F), UINT8_C(0x44), UINT8_C({CompilerContract.ManagedModuleAbiVersion}), UINT8_C(0), UINT8_C(0) }},");
    ...
    public static string OverlayBodyName(MethodSymbol method, string callableName) =>
        NameMangler.Artifact("ct_ob_", $"overlay-body:{method.OverlayName}:{NameMangler.MethodIdentity(method)}:{callableName}");
    ...
    {
        if (method.IsOverlay)
            throw new InvalidOperationException("Overlay methods cannot be emitted as unmanaged function-pointer trampolines.");
        var key = (type, method);
