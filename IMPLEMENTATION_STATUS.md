...
The .NET 10 build uses SDK `10.0.400-preview.0.26322.102` and completes with zero warnings and zero errors. All 116 registered managed and native conformance checks pass. Type-body completion again includes the `operator` declaration keyword; operator hover, definition, document/workspace symbols, semantic classification, and exclusion from ordinary member completion remain covered. Draft 0.14 coverage includes deterministic C~ debug mappings and maps, runtime-fault catches and injected OOM, lifecycle finalization and panic callbacks, deterministic modular artifacts and symbol maps, unity/modular native behavior, contiguous storage, constructive `out`, BVH/list hit equivalence, AABB edge rays, schedule-independent per-sample RNG, and the exact reduced-image SHA-256 `5709717E43C2752ECE14180A8B5E424B96638D7E34FA726CC60248DDEAB121DF`.
...
| C~-aware native debugging | Implemented | Source-only and instrumented modes, version-2 logical probes and scopes with optional bulk target layouts, cached C~ stepping and inspection, Run to Cursor, logical exception/log/function breakpoints, GDB watchpoints, ARC runtime inspection, ESP target-output forwarding and detach-and-continue, validated descriptors, WSL and ESP UART-stub resolution, and MSVC fallback |
...
The executable test project registers 116 checks, and all 116 pass. Coverage includes:
...
Typed IR contains typed values, basic blocks, loads, stores, calls, allocations, conversions, checks, ownership and cleanup actions, and structured terminators. Reachability, direct-defer cleanup facts, leaf cleanup elision, durable-state liveness, fused scalar string builds, and user metadata pruning run before artifact layout. `TypedIrEmissionLowerer` then creates immutable function and initializer plans only for retained IR, and `CEmitter` composes those plans without reopening method syntax. The former rendered-line classifier, `MethodLowerer`, `BodyPipeline`, `CBodyLowerer`, and `LoweredExpression` transition layers are gone. `GetDiagnostics()` is analysis-only and constructs no `CEmitter`, `CWriter`, typed IR, or generated C.
...
On 2026-08-22, an instrumented debugger-v2 image was flashed to the connected dual-core ESP32 on COM4. The DAP connected through ESP-IDF's Xtensa GDB at 460800 baud, hit the logical C~ breakpoint at `Program.ct:299`, forwarded C~ console output through GDB target-output packets, and completed the serialized detach-and-continue path. The 2026-08-21 source-mapped image separately enumerated five FreeRTOS tasks, exposed C~ locals, stepped, classified a handled `CTE0001` hook stop, and confirmed continued WS2812 output after detach. Host probe and stepping behavior is also validated under WSL GDB; the connected startup-gate, watchpoint, and full ARC-inspection matrix remains open. This does not close the complete ABI 14 firmware-runtime gate below.
...
The software gates currently pass. The release remains blocked because ABI 14 has not yet completed its physical-board flash-and-monitor validation.
