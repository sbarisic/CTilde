...
$destination = Join-Path ([System.IO.Path]::GetFullPath($OutputDirectory)) 'CTilde.VisualStudio-0.12.0.vsix'
...
$inventoryPath = Join-Path ([System.IO.Path]::GetFullPath($OutputDirectory)) 'CTilde.VisualStudio-0.12.0.inventory.txt'
...
[System.IO.File]::WriteAllLines((Join-Path ([System.IO.Path]::GetFullPath($OutputDirectory)) 'CTilde.VisualStudio-0.12.0.sha256.txt'), $metadata, [System.Text.UTF8Encoding]::new($false))
$metadata
