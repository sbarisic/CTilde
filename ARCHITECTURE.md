...
For overlays, typed-IR reachability assigns each executable body to resident storage or one named placement. Runtime support, descriptors, statics, strings, constants, type operations, lifecycle code, export/import stubs, cleanup callbacks, and native sources remain resident. Same-overlay direct calls retain their body target; every uncertain or crossing dispatch uses the stable stub. The native build first links a complete ELF, then the packager collects overlay text, literal pools, and audited relocations, removes those bytes and relocation records from loadable ELF segments, appends a deterministic hashed directory/container, and atomically publishes the `.ctm`.

The loader retains a locked read-only package stream for every module with overlays. Each process lazily allocates one executable window sized to the largest overlay in its exact dependency closure. A transition reads directly into the window, validates SHA-256, applies window or load-bias relocations, clears the tail, flushes data and instruction visibility, and publishes a new generation. A nested return reloads the caller before execution resumes. Any failure after the previous payload may have been overwritten terminates that process entirely from resident code. This intentionally trades transition I/O for a one-window peak executable-memory bound and does not cache, prefetch, compress, or pin payloads.
...
The managed-module host replaces the single-program assumption with registry-owned module descriptors and process instances. The loader-created managed main task has an execution context containing its process, active module call stack, active overlay and window generation, runtime-operation depth, and runtime thread state. Normal completion finalizes process-local module statics in reverse dependency order, drains tracked allocations and messages, closes retained overlay streams, releases the application and newly unused dependencies in reverse order, then signals completion. Draft 0.48's three bounded redirected stream buffers and reclaimable monotonically assigned process identities remain unchanged. Forced termination is submitted to a dedicated control task, which closes an atomic stop-bit/operation-count gate before deleting the main task at a stable runtime boundary. Physical overlay-source loss marks matching streams unavailable and forces affected processes to resident cleanup without running user finalizers. Source-created child-task ownership and the complete native-resource ledger remain outstanding.
