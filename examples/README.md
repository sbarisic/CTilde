...
Run the hosted/MSVC catalog smoke from the repository root with `./Test/Test-ExampleCatalog.ps1`. It compares the complete deterministic output of both focused hosted tours, executes the MSVC native-import plug-in, and performs compiler checks for both managed-module projects. Add `-IncludeEspIdfBuild` (and, if needed, `-IdfPath <path>`) to build the real `.ctm`, `.ctmeta.json`, and ManagedShell firmware artifacts. Setting `CTILDE_EXAMPLE_ESP_IDF_BUILD=1` enables the same lane inside the Release validation runner; it uses `IDF_PATH` or the checked default installation path. Hardware execution and the remaining target-specific runners stay separate.
