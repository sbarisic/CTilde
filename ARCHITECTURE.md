...
Draft 0.45 retains the single typed-IR and C backend while adding a distinct ESP-IDF managed-module artifact plan and a firmware-owned shared Runtime ABI 18 component. Ordinary hosted, Cosmopolitan, freestanding, and ESP-IDF firmware artifacts retain their existing startup model. Draft 0.44 remains the verified-stack-contract and dependency-oriented modular-emission revision; the target-specific Cosmopolitan design remains in [COSMOPOLITAN.md](COSMOPOLITAN.md).
...
This follows ESP-IDF's source-compatibility boundary. Native configuration structures, enum numbers, and typedef implementation details do not become durable compiler metadata. Opaque returns carry explicit owned/borrowed and nullable contracts, so ordinary C~ ownership lowering checks their cleanup without exposing native layouts. Private, `esp_private`, example, preview, and experimental headers require an explicit unstable opt-in. Generated adapter symbols are project-private and do not alter Runtime ABI 18 or the public native header.
