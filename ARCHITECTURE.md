...
Draft 0.49 retains the single typed-IR and C backend. Runtime ABI 22 and Managed Module ABI 3 add cleanup-safe provider call stubs and process-local ESP32/Xtensa code overlays on top of Draft 0.48 redirected streams, stable process identities, metadata-only dependencies, and checked managed calls. The Draft 0.46 block-device/FAT and process-owned VFS architecture remains intact. Ordinary hosted, Cosmopolitan, freestanding, and ESP-IDF firmware artifacts retain their existing startup model.
