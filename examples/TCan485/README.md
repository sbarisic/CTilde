...
Run it from the repository root. The normal command prompts once to confirm visible WS2812 activity. `-AutomatedOnly` performs all machine-verifiable checks but leaves that release gate pending. The runner also flashes `Draft018Validation.ct`: native `xTaskCreate` starts the generated `[TaskEntry]` wrapper, ordered and relaxed MMIO access a shim-owned RAM word, typed extern data carries the result, and the task reports its architecture and stack headroom before the fixture emits `CTILDE_DRAFT_018_OK`. `-AcceptMemoryBaseline` is an explicit reviewer-controlled update; ordinary runs only validate the tracked versioned baseline and fail when the ESP-IDF or compiler version differs. Each run writes an ignored JSON report, raw UART bytes, and transcripts under `artifacts/esp32-hardware`.
