...
Run it from the repository root. The normal command prompts once to confirm visible WS2812 activity. `-AutomatedOnly` performs all machine-verifiable checks but leaves that release gate pending. The runner also flashes the versioned Draft 0.18, 0.19, and 0.20 fixtures. `Draft018Validation.ct` checks task-entry attachment, MMIO, native data, architecture, and stack headroom. `Draft019Validation.ct` checks constant layouts, newtypes, recursion restrictions, and CPU operations. `Draft020Validation.ct` checks endian wire values and scalar bitfield views before it emits `CTILDE_DRAFT_020_OK`. `-AcceptMemoryBaseline` is an explicit reviewer-controlled update; ordinary runs only validate the tracked versioned baseline and fail when the ESP-IDF or compiler version differs. Each run writes an ignored JSON report, raw UART bytes, and transcripts under `artifacts/esp32-hardware`.
