...
Draft 0.45 introduces Runtime ABI 18 and Managed Module ABI 1 for trusted ESP-IDF ELF applications that bind to the firmware-owned process host and runtime-service table. Draft 0.44 remains the verified native stack-contract and dependency-oriented modular-C revision. Debug metadata remains version 3.
...
An ESP-IDF project selects `espIdf.artifact: "managed-module"` and `build.cLayout: "modules"`. Its `managedModule` block contains `kind` (`application` or `library`), a canonical name, an exact version, exact `.ctmeta.json` references, the application task stack size, and an optional heap limit. Canonical names are ASCII and at most 63 bytes; exact versions are ASCII and at most 31 bytes, leaving one NUL byte in each fixed ABI field. The same limits apply to every referenced identity and dependency. Build emits `<name>.ctm`, a 32-bit target ELF shared object, and deterministic `<name>.ctmeta.json` public metadata. The ELF contains a fixed binary preflight manifest for Runtime ABI 18 and Managed Module ABI 1. It emits neither `app_main` nor a private copy of the firmware module/process host. The current implementation still emits reachable compiler-generated language-runtime and standard-library helpers in each module; complete shared-code extraction remains implementation work.

Managed Module ABI 1 is ESP-IDF-only in Draft 0.45. A hosted application can resolve ordinary native C ABI methods through `[NativeImport]`, but the hosted target does not emit `.ctm` files, consume `.ctmeta.json` references, or register managed C~ module types in a shared desktop runtime.

Every public type and member of a managed library is recorded in its managed ABI metadata. Public signatures may use scalars, enums, strings, arrays, concrete classes and structures, interfaces, and delegates. Public generic definitions and constructed generic types are rejected. `internal` is reserved as binary-module-local access. Draft 0.45 does not yet consume provider metadata, patch managed import slots, or execute cross-module managed calls. The complete Managed Module ABI will allow managed references to cross module boundaries only inside one logical process; process messages are copied byte arrays and never transfer managed identity.

The firmware owns one versioned `ct_runtime_api_v18` table, process registry, and module registry. A module binds the table through its fixed entry function. Mutable statics are stored in per-process module instances; code and immutable metadata are shared. Applications run as logical processes backed by one managed main FreeRTOS task. Arguments are copied before entry. Canonical cross-module type registration and translation of an unhandled managed exception into a failed process exit remain incomplete; current applications must not rely on an exception boundary to protect the firmware.

Module paths name direct children of `/storage/modules`, either by filename or by an absolute path with exactly that parent. Empty names, `.`, `..`, nested paths, backslashes, and absolute paths outside that root are rejected. This flat namespace matches the current Espressif loader's basename-keyed module registry and ensures preflight and relocation select the same file. Dependency metadata names an exact canonical name, version, build identity, and API hash. The current structural loader recursively preflights exact dependencies, rejects cycles and global version/build/API conflicts, creates process-local module instances in dependency order, and releases newly unused modules in reverse order. It also pins modules across dependent load references and tracked active calls, type registrations, and allocations. This is loader scaffolding rather than a supported managed-library programming surface: the compiler does not yet consume provider declarations, bind managed import slots, or account for cross-module objects, vtables, delegates, callbacks, and function leases.

Current processes have independent module instances, mutable statics, managed allocation lists, heap accounting, cancellation state, one managed main task, and copied-message mailboxes. `Cancel` is cooperative. `Terminate` requests cancellation and may delete that main task after its grace period; `uint.MaxValue` selects an unbounded cooperative wait. A dedicated control task owns deletion and a reaper performs blocking cleanup outside the task-deletion callback. Source-created child-task ownership and the complete native-resource ledger are not implemented. Forced reclamation therefore covers tracked managed allocations and host bookkeeping only. Because modules are trusted native code and ESP32 provides no process memory protection, raw pointers, locks, callbacks, interrupts, MMIO, child tasks, or untracked native resources that escape accounting make forced termination behavior undefined.
