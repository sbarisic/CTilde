...
Debug information is additive and does not change runtime ABI 14. Debug-enabled output may contain `#line` directives and private non-inlined `ct_debug_throw_hook` and `ct_debug_fatal_hook` functions. They are compiler/debugger implementation details, are absent from ordinary Release emission, and are not exported native contracts. The separate version-1 debug map is tooling metadata, not a link-time ABI artifact.
