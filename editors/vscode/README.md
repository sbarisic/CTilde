...
- Current language syntax and semantic classification, including `[Overlay]`, `[Resident]`, `System.Storage`, Runtime ABI 23 managed process and filesystem services, Module ABI 4 metadata references, the `size` optimization profile, `[NativeImport]`, ESP-IDF managed-module manifests, `simdOptimizations`, `Vec3x4`, generic collections, indexers, iterators, explicit SIMD128 values, scalar geometry, repository modules, source owners, closures, interrupts, effect contracts, target profiles, runtime roles, and native interop.
