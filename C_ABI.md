...
Generated files include C standard-library headers and the platform headers required by reachable services, such as Windows, pthread, or FreeRTOS threading. Compile-time assertions require:
...
`Compilation.EmitSymbolMap`, CLI `--symbol-map`, and modular bundles emit version 1 JSON sorted by compact name. Each entry includes the compact name, full canonical identity, kind, signature/result type, and source location. The map declares Runtime ABI 23.
...
The [ManagedShell example](examples/ManagedShell/README.md) contains separate solution projects for the firmware host and its loadable applications and libraries. The diagnostics tools use the example-local `ct_managed_diagnostics_host_v1()` accessor with protocol version 2. `shell.ctm` uses a separate copied-snapshot/foreground-control accessor. `system.ssh.ctm` uses `ct_managed_ssh_host_v2()`, whose generation-tagged opaque tokens cover sockets, hash operations, P-256/X25519 keys, and AES-GCM contexts. The firmware stores only resident cleanup callbacks in the process resource ledger; it never stores a module callback or exposes an ESP-IDF descriptor or MbedTLS pointer. These accessors are example-local contracts, not Runtime ABI 23 or Managed Module ABI 4.

## Firmware capability tables

The [shared contract header](runtime/esp-idf/ctilde_managed_runtime/include/ct_runtime_contract.h) defines runtime declarations and capability identifiers. The compiler embeds this header into generated managed C.

`GetCapability` returns an immutable firmware-owned table, or null when the identifier, major version, or minimum size is unsupported. Every table starts with `Size` and `MajorVersion`. The loader checks required capabilities before module initialization. Generated bindings cache the core and buffer tables only after validation succeeds.

Core services preserve allocation ownership and process context. Buffer services provide memory operations, byte hashing, UTF-8 operations, and integer formatting. Network and crypto tables use opaque resource handles and caller-owned buffers. Filesystem calls preserve the existing file-service ownership and runtime-operation checks. Process services expose current process identity, cancellation state, delay, time, and descendant termination.

The registry closes to new registrations at its first lookup. Table storage must remain valid for the firmware lifetime. Capability discovery does not create a security sandbox. Compiler-generated network, crypto, filesystem, and process calls remain unfinished. SSH still uses its native adapter.

Shared helpers do not contain caller exception landing points. They do not retain caller buffers. UTF-8 encoding requires a validated scalar and four writable bytes. Integer formatting requires 20 bytes for unsigned values or 21 bytes when a sign is requested. Formatting returns the byte count without a terminating zero.
