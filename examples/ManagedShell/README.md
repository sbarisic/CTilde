...
ManagedShell is the in-progress Draft 0.51 ESP-IDF example for Runtime ABI 23 and Managed Module ABI 4. `Examples.sln` exposes the firmware, its applications, libraries, and overlay acceptance fixtures as separate projects under **Managed Modules**. The firmware mounts LittleFS at `/storage` and the isolated SFTP partition at `/sftp`, owns the T-CAN485 removable-SD monitor for `/sd`, initializes the shared process/runtime-service hosts, and supervises `/storage/modules/shell.ctm --uart`. The same loadable shell application supplies UART, redirected SSH, and one-command exec sessions.
...
With an existing server and a locally unlocked client key, run the acceptance probe from the repository root:

```powershell
python Test/Test-ManagedSsh.py --host 192.168.1.48 --identity "$env:USERPROFILE/.ssh/ctilde_esp32" --known-hosts "$env:USERPROFILE/.ssh/known_hosts" --output artifacts/ssh-acceptance
```

Use the device's current address and a known-hosts file containing its verified host key. The probe does not request or store passphrases. It checks a remote `free` command, interactive sessions, input EOF, and immediate reconnect. Add `--sftp` only for the optional development profile to test an upload/download and hash comparison. Results and timeout details are written to `ssh-acceptance.json`. These basic checks do not establish foreground cancellation, maximum-packet, or stack acceptance.

`Test/Test-ManagedSshControls.py` adds resize, fragmented terminal input, prompt Ctrl+C, rekeying, stalled output, UART responsiveness, and nonzero exit-status checks. It requires Paramiko 5 and pyserial. Close the serial monitor before running it. Supply `--host`, `--known-hosts`, `--fingerprint` (the SHA256 fingerprint from `ssh-add -l`), `--port`, and `--output`. The test uses the unlocked agent key, verifies the server key, and does not change device files or credentials. Prompt Ctrl+C does not establish foreground-child cancellation.
...
`system.ssh.ctm` is a metadata-linked Managed Module ABI 4 library. `sshd.ctm` compiles against its deterministic `.ctmeta.json` declarations without provider source, obtains the fixed administration defaults from `SshDaemon.ManagedShellDefaults()`, and enters the library service loop. The library implements one nonblocking connection, strict Curve25519 key exchange, P-256 host signatures and public-key authentication, AES-128-GCM packets, rekey requests and one-hour/1-GiB limits, two session channels, redirected shell/exec, window changes, and channel flow control. The default source profile rejects subsystem requests. The optional `Modules/SystemSsh/ctilde.sftp.json` profile adds SFTP v3 rooted at `/sftp`. Service, transport, channel, configuration, key-exchange, and authentication groups are separately placed; stable calls and the active connection state remain resident. The firmware exports only a versioned opaque-token socket/crypto accessor whose resources are charged to the `sshd` process.

Start the service as `sshd.ctm &`. Its mailbox accepts `status`, `reload-keys`, `disconnect`, and `stop`. The configured shell path is fixed to `/storage/modules/shell.ctm`, bypassing SD module precedence for the administration shell. Reload constructs a complete replacement key set; an active connection keeps its captured keys until it disconnects. OpenSSH public-key authentication, remote commands, interactive sessions, EOF, and reconnect pass on the ESP32 RAM-loaded candidate. Additional device checks cover resize, rekeying, and stalled output. The full SSH checkpoint, packet-pressure tests, endurance, and independent security review remain pending. SFTP is excluded from default acceptance. See [current validation](SSH_XIP_VALIDATION.json) and [remaining gates](SSH_XIP_PROGRESS.md) for the tested scope. Do not treat it as production-ready or expose it to an untrusted network.
...
See [current implementation and validation limits](SSH_XIP_PROGRESS.md) for the work based on `cda18458`.
SSH requests 1 KiB stdin, stdout, and stderr pipes. Ordinary process callers retain the 8 KiB defaults.
The default module excludes SFTP implementations. To build the optional development profile, use:

```powershell
ctilde --project examples/ManagedShell/Modules/SystemSsh/ctilde.sftp.json --build --idf-path C:/esp/v6.0.2/esp-idf
```

The profiles share build outputs. Rebuild `ctilde.json` before packaging the default firmware and module set.
The partition table remains unchanged. Neither profile enables XIP.
