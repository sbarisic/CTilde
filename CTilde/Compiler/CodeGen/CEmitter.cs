...
internal sealed class CEmitter
{
    ...
    {
        writer.WriteLine("/* Generated by C~ draft 0.3. Do not edit. */");
        ...
        writer.WriteLine();
        writer.WriteLine("_Static_assert(CHAR_BIT == 8, \"C~ requires 8-bit bytes\");");
        writer.WriteLine("_Static_assert(sizeof(int32_t) == 4 && sizeof(uint32_t) == 4, \"C~ requires exact 32-bit integers\");");
        writer.WriteLine("_Static_assert(sizeof(float) == 4 && FLT_RADIX == 2 && FLT_MANT_DIG == 24, \"C~ requires IEEE-754 binary32 float\");");
        writer.WriteLine("_Static_assert(INT32_MIN == (-2147483647 - 1), \"C~ requires two's-complement int32_t\");");
        writer.WriteLine();
        ...
        writer.WriteLine("}");
        writer.WriteLine("static void ct_write_string(const ct_string* value) { if (value != NULL && value->Length > 0) (void)fwrite(value->Data, 1u, (size_t)value->Length, stdout); }");
        writer.WriteLine("static void ct_write_char(uint8_t value) { (void)fputc((int)value, stdout); }");
        writer.WriteLine("static void ct_write_int(int32_t value) { (void)fprintf(stdout, \"%\" PRId32, value); }");
        writer.WriteLine("static void ct_write_uint(uint32_t value) { (void)fprintf(stdout, \"%\" PRIu32, value); }");
        writer.WriteLine("static void ct_write_float(float value) { (void)fprintf(stdout, \"%.9g\", (double)value); }");
        writer.WriteLine("static void ct_write_bool(bool value) { (void)fputs(value ? \"True\" : \"False\", stdout); }");
        writer.WriteLine("static void ct_environment_exit(int32_t code) { exit((int)code); }");
        writer.WriteLine();
    }
    ...
    private void EmitKeepSymbols(CWriter writer)
    {
        ...
        var runtime = new[]
        {
            ...
            "ct_write_string", "ct_write_char", "ct_write_int", "ct_write_uint", "ct_write_float", "ct_write_bool",
            "ct_environment_exit",
        };
