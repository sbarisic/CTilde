...
These aliases force ESP32/Xtensa and ESP32-C3/RISC-V 32, use `build/esp32_qemu` and `build/esp32c3_qemu`, and keep target-specific `sdkconfig` and component lock files. Their `static if (Target.Environment == TargetEnvironment.Qemu)` branch runs deterministic managed-runtime, ARC, exception, task/TLS, callback, timer, supported assembly, Draft 0.40 PCG32/Stopwatch checks, and Draft 0.42 integer, hexadecimal, floating-point, and enum parsing checks. It prints `draft40 foundations: ok`, `draft42 parsing: ok`, and `CTILDE_ESP_QEMU_OK`, then returns. It never executes RMT, Wi-Fi, CAN, USB, watchdog, hardware RNG, or board-specific memory access. QEMU therefore validates compiler/runtime behavior, not T-CAN485 peripheral compatibility.
...
Run it from the repository root. The normal command prompts once to confirm visible WS2812 activity. `-AutomatedOnly` performs all machine-verifiable checks but leaves that release gate pending. The runner also flashes the versioned Draft 0.18, 0.19, 0.20, and 0.23 fixtures. `Draft018Validation.ct` checks task-entry attachment, MMIO, native data, architecture, and stack headroom. `Draft019Validation.ct` checks constant layouts, newtypes, recursion restrictions, and CPU operations. `Draft020Validation.ct` checks endian wire values and scalar bitfield views. `Draft023Validation.ct` registers an IRAM C~ GPTimer ISR, updates volatile DRAM state, crosses an interrupt-safe native acknowledgement, and emits `CTILDE_DRAFT_023_OK` after both counters agree. `-AcceptMemoryBaseline` is an explicit reviewer-controlled update; ordinary runs only validate the tracked versioned baseline and fail when the ESP-IDF or compiler version differs. Each run writes an ignored JSON report, raw UART bytes, and transcripts under `artifacts/esp32-hardware`.
...
That physical result predates the Wi-Fi/HTTPS worker. The normal firmware now includes and invokes that worker, while the clean-checkout image skips native network initialization because its tracked SSID is empty. ESP-IDF Wi-Fi, TLS, the HTTP client, and the full certificate bundle remain linked in both cases. The deliberately rebaselined ESP32 cross-build is 1,010,400 binary bytes and 1,010,276 image bytes: 696,742 bytes flash code, 204,764 bytes flash data, 90,779 bytes IRAM, and 38,951 bytes static DRAM. The ESP32-C3 cross-build is 1,072,624 binary bytes and 1,072,254 image bytes: 776,674 bytes flash code, 207,020 bytes flash data, and 109,568 bytes static DRAM. These are the tracked ESP-IDF 6.0.2/GCC 15.2.0 limits; a live-network acceptance run still requires local credentials because credentials are never tracked.

Draft 0.44 Runtime ABI 17 completed connected-board acceptance on 2026-09-01. The ordinary ESP32 image measured 1,010,400 binary bytes and 1,010,276 image bytes and reported 268,144 bytes free heap, a 249,236-byte minimum, and 6,804 bytes of main-task stack headroom. The ABI fixture recorded the new standard-library array and exception layouts, a 16-byte vtable, and totals of 1,000 descriptor bytes, 304 vtable bytes, and 496 literal-object bytes. Firmware, Draft 0.18 through 0.23, allocation failure, exact console bytes, fatal/restart/halt policies, debugger-v3 inspection, four post-detach transitions, and the 14.53-second startup release all passed. The runner restored the ordinary Release firmware, and the ignored automated report is `artifacts/esp32-hardware/20260901-215707.json`. After restoration, the operator confirmed that the ordinary firmware visibly blinked the onboard LED, completing the visual gate.

An automated connected-board pass of that new network-disabled image completed on 2026-08-23. It printed `wifi: not configured`, completed all ABI markers and 25 alternating WS2812 transitions, and measured 291,696 bytes free heap, a 278,964-byte minimum, and 6,708 bytes of main-task stack headroom. The allocation-failure, exact-console, fatal-reset, debugger-v3, detach, and startup-timeout checks also passed. The ignored report is `artifacts/esp32-hardware/20260823-211534.json`. Because the run used `-AutomatedOnly`, the previous visual LED confirmation remains the latest human observation; no live HTTPS claim is made without local credentials.

A Draft 0.23 automated connected-board pass completed on 2026-08-28. The GPTimer fixture invoked the direct C~ interrupt entry from IRAM, updated compiler-owned DRAM state, crossed the explicitly trusted interrupt-safe acknowledgement boundary, and emitted `CTILDE_DRAFT_023_OK` after the counters agreed. The run also passed the ordinary ABI workload, Draft 0.18 through 0.20 fixtures, memory, exact-console, panic-policy, debugger, detach, and startup-timeout gates. It restored the ordinary Release firmware before completion. The ignored report is `artifacts/esp32-hardware/20260828-000319.json`. After restoration, the operator confirmed that the onboard LED visibly alternated between blue and purple, closing the visual gate that the `-AutomatedOnly` report left pending.
...
The current runtime uses atomic ARC and per-task exception, cleanup, diagnostic-origin, and release state; cycles leak, and cleanup is not promised after a panic, fatal failure, or reset. The example reserves its configured FreeRTOS application TLS slot through `sdkconfig.defaults`. Native buffers and UTF-8 views remain scoped synchronous values and cannot be retained by the shim. Opaque ownership is lexical; long-lived resource fields are not supported. The WS2812 API owns one native strip handle for firmware lifetime. Native-created tasks must call `ct_thread_attach` and `ct_thread_detach`; source-created `Thread` workers and `[TaskEntry]` wrappers attach automatically. Draft 0.23 interrupt entries do not attach to the managed runtime and may use only the compiler-checked runtime-free, non-blocking IRAM/DRAM closure. Retained callbacks remain unsupported. Permanent loops must keep live ownership bounded and call a yielding API such as `DelayMilliseconds`; a busy loop can trigger the ESP-IDF task watchdog.
