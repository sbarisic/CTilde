...
C~ draft 0.39 has one compiler path:
...
The compiler library, CLI, and conformance runner target .NET 10. Draft 0.39 adds hosted native-import startup resolution and deterministic library lifetime on top of Draft 0.38 hosted x64 scalar-geometry SIMD optimization, `System.Simd.Vec3x4`, and four-ray HostedIo production batching. HostedIo now displays progressive RGBA output through a minimal Raylib 6.0 native-import surface on Windows x64 and Linux x64. Its manifest selects the checked-in official runtime payload through `hosted.runtimeFiles`; the CPU image receives each completed pixel immediately while dirty uploads and event pumping are capped at 60 Hz. Ordinary project loading is network-free and uses replacement, verified-vendor, then exact-cache precedence.
