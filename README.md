...
- `Examples.sln` contains 20 projects grouped as language and hosted programs, systems targets, managed modules, and T-CAN variants. It includes separate editor projects for the ManagedShell firmware, Hello application, memory tool, and task manager.
...
`Test/Test-ExampleCatalog.ps1` compares the complete output of the focused hosted tours, checks the ManagedShell firmware plus its Hello, memory, and task-manager module projects, and executes the native-import example. `-IncludeEspIdfBuild` adds all three `.ctm` files, their metadata, and firmware packaging when ESP-IDF is installed; `CTILDE_EXAMPLE_ESP_IDF_BUILD=1` enables that lane through the Release validation tier. The ordinary portable smoke remains ahead of the larger compiler and SIMD matrices.
