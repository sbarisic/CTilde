...
Draft 0.12 adds body-bearing `System.Vec2`, `System.Vec3`, and `System.Vec4` declarations. Compilation detects exact vector identifiers and loads only the corresponding embedded source, while language-service snapshots load all three for discovery and navigation. Draft 0.13 adds a raw inline-assembly syntax node, typed operand bindings, a side-effecting typed-IR instruction, and GNU extended-asm rendering. Draft 0.14 replaces the process runtime, storage layouts, module lifecycle, and generated names. Draft 0.15 adds abstract/interface dispatch, closed generic monomorphization, scalar atomic and volatile operations, and managed Thread/Mutex/lock lowering. Draft 0.16 adds union, packed, and explicit aggregate metadata, one shared C layout renderer, generated access paths, and symbolic layout operators. Draft 0.17 adds immutable native-section metadata on method and field symbols, generic-clone propagation, deterministic placement macros, and matching unity, modular, and public-header annotations. Assembly text and section names never become C identifiers or ordinary expression fragments.
