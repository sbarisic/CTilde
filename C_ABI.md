...
The [ManagedShell example](examples/ManagedShell/README.md) contains separate solution projects for the firmware host and its loadable applications and libraries. The two diagnostics modules use the example-local `ct_managed_diagnostics_host_v1()` accessor with protocol version 2. Its fixed-width copied heap/task records and wrapper functions deliberately hide ESP-IDF and FreeRTOS structure layouts from module C. This accessor is not part of Runtime ABI 22 or Managed Module ABI 3.
