...
- `CEmitter` owns reusable runtime, public-header, internal-header, source-owned, entry/lifecycle, symbol-map, and CMake fragments plus hosted or ESP-IDF policy. Unity and modular layouts consume the same reachable optimized `TypedIrProgram`; partitioning never changes semantic lowering. `[Used]` declarations receive target-specific final-image retention directives, while ordinary translation-local definitions remain eligible for removal.
