...
Draft 0.50 retains the single typed-IR and C backend, Runtime ABI 22, and Managed Module ABI 3. It adds a controlled size profile, call-graph placement of private single-overlay helpers, and split packaged-ELF load segments so loader metadata and data do not consume executable memory. Draft 0.49 cleanup-safe provider call stubs and process-local ESP32/Xtensa code overlays remain intact on top of Draft 0.48 redirected streams, stable process identities, metadata-only dependencies, and checked managed calls. The Draft 0.46 block-device/FAT and process-owned VFS architecture remains intact. Ordinary hosted, Cosmopolitan, freestanding, and ESP-IDF firmware artifacts retain their existing startup model.
...
UTF-8 source files
    ...
    -> External C compiler
    -> Native executable, firmware component, or ESP-IDF .ctm module
```
...
Hosted and freestanding object-cache format 2 runs a compiler dependency scan before lookup: MSVC uses `/Zs /sourceDependencies`, and GNU-family compilers use `-M`. Keys include resolved transitive headers, source identity, working directory, flags, include environment, target configuration, and compiler version and binary hash. WSL dependencies are hashed in WSL. Missing or unusable scan results select an uncached compilation. A successful compile publishes the object atomically; failed compiles leave no reusable candidate. This private format version is independent of language and runtime ABI versions.
...
Source creation and deletion re-expand manifest globs; a rename is a deletion plus a creation. Unchanged syntax trees are reused by normalized path, source content, and parse origin, including generated bindings. Semantic analysis is rebuilt for each new snapshot. Project resets evict obsolete cached trees while existing immutable snapshots remain valid. File URIs use authority-aware local paths, including Windows UNC servers and escaped characters. Publication checks reject snapshots superseded by a buffer or membership change.
...
`Examples.sln` is a discovery surface, not a cross-target build graph. Its 27 C~ projects are grouped as language/hosted programs, systems targets, managed modules, and T-CAN variants; none has a `Build.0` mapping. The managed group contains separate firmware, application, library, administration-tool, and overlay-acceptance projects. The [example catalog](examples/README.md) records each project's toolchain or hardware prerequisite. Focused hosted examples execute common features without optional hardware, while target examples retain their own explicit runners.
...
Emission lazily lowers the already validated bound program to semantic typed IR, computes a reachability closure, and creates immutable emission plans for the retained functions and module initializers. The typed-IR optimizer attaches deterministic facts for cleanup-record liveness, constants, non-null values, and movable owned results; emission dataflow additionally tracks fixed array lengths. Emission consumes those facts to omit empty lexical cleanup boundaries, move fresh ownership into strong slots and return payloads, remove immortal-string ARC traffic, elide only dominated null and fixed-index bounds checks, and simplify constant loops and valid constant `stackalloc` sizes. A shared exact-dispatch predicate also emits direct methods, accessors, indexers, and delegate thunks for sealed static class receivers and sealed overrides while leaving interface and unsealed receivers virtual. Facts are invalidated conservatively at `ref`/`out` aliases, unsafe calls, inline assembly, fields, nullable native results, and control-flow joins; dynamic safety checks remain when proof is incomplete. Instrumented output retains its logical source probes and lexical storage even when a neighboring native operation disappears.

`System.String` is recognized only from the selected embedded or physical standard library. Binding aliases it to the built-in `string` type, makes its members available to ordinary overload resolution and editor services, and rejects user storage, constructors, or layout changes. Emission keeps the existing `ct_string` object layout and supplies its interface table without generating an ordinary class payload. Search, checked construction, native UTF-8 conversion, builder, and formatting helpers join the same reachability closure; unused programs do not receive those helpers or the vendored Ryu tables.

Draft 0.42 applies the same compiler-owned surface model to Boolean, fixed-width and native-width integers, and binary32/binary64. Keyword and `System.*` spellings resolve to one scalar identity; only static library methods are permitted and no class storage or descriptor is emitted. Integer and Boolean parsers are target-neutral generated helpers. Reachable floating parsers embed the pinned Ryu `s2f`/`s2d` implementation. Closed enum parsing registers a deterministic per-enum helper containing only reachable names and the exact underlying bounds.

Hosted I/O remains isolated behind compiler-owned extern names. The public move-only file handle wraps a private native `FILE*`; managed streams keep the handle in private native-width storage and close it exactly once. Windows path operations validate UTF-8 and convert to UTF-16 before Win32 calls. Linux and Cosmopolitan use validated zero-terminated UTF-8 and POSIX calls. Directory enumeration normalizes results into managed full-path strings and sorts them ordinally before returning. All support is emitted from the same reachability closure, so parse-only and I/O-free programs do not acquire filesystem or parser helpers.

The artifact emitter composes only retained plans. Unity concatenates the fragments. Modular output shares runtime and internal headers, writes one runtime source, partitions definitions by canonical source identity, and writes an entry/lifecycle source, symbol map, and CMake source fragment. The broad internal header carries only the reachable translation-unit-local inline atomic, wrapping-arithmetic, ARC, null, bounds, and stack-size fast paths; cold failures and larger runtime operations retain one runtime definition. A body-only edit therefore changes only its owning source artifact. Direct `defer` records avoid exception frames, and fused scalar string builds evaluate segments once from left to right. Calling `GetDiagnostics()` never initializes typed IR, a C writer, or emitter artifacts.
...
This follows ESP-IDF's source-compatibility boundary. Native configuration structures, enum numbers, and typedef implementation details do not become durable compiler metadata. Opaque returns carry explicit owned/borrowed and nullable contracts, so ordinary C~ ownership lowering checks their cleanup without exposing native layouts. Private, `esp_private`, example, preview, and experimental headers require an explicit unstable opt-in. Generated adapter symbols are project-private and do not alter Runtime ABI 19 or the public native header.
...
Draft 0.45 adds a separate managed edge for ESP-IDF only. Firmware owns the module and process registries, runtime table, task-local process context, allocation ledger, and ELF loader. Draft 0.49 Module ABI 3 embeds exact dependency/export/import tables and mutable placement-aware call targets. The compiler loads schema-3 provider declarations as dependency-owned semantic trees without compiling provider source, excludes those declarations from consumer export roots, and emits checked slots pointing to provider-owned stable resident stubs. The loader resolves all slots before publication. A stub pushes a resident cleanup callback, enters the target through Runtime ABI 22, and invokes the patched body; normal return and exception unwinding therefore restore identical active-call, module-instance, and overlay state. Stable type fingerprints and sized `ct_type_ops` are present, but generated objects and dispatch metadata do not yet replace every local descriptor with the canonical runtime-returned descriptor, shared unboxed generic implementations remain pending, and reachable standard-library helpers can still be duplicated in modules. This architecture is independent of hosted `[NativeImport]`, whose types and ownership stop at the native C ABI.

For overlays, typed-IR reachability assigns each executable body to resident storage or one named placement. Runtime support, descriptors, statics, strings, constants, type operations, lifecycle code, export/import stubs, cleanup callbacks, and native sources remain resident. Typed call lowering sends proven same-overlay calls directly to the ephemeral body; every uncertain or crossing dispatch uses the stable stub. The native build relinks with Xtensa relaxation disabled and static relocations retained, rejects instruction targets outside their owning overlay, then removes overlay text, literals, and audit relocations from the resident ELF. It appends a deterministic hashed directory/container and atomically publishes the `.ctm`.

Semantic binding records exact delegate and unmanaged-address targets separately from execution edges. Delegate targets keep callable resident entries; an unmanaged-address target and its ordinary call closure remain resident. Taking an unmanaged address of an explicitly overlaid method remains invalid. Address formation alone does not add a call to effect analysis.

Each managed process has a private allocation lock initialized on slot reuse. Allocation reserves payload bytes against its quota under that lock before calling the allocator. Failure rolls back the reservation; success publishes the allocation record under the same lock. Freeing unlinks and accounts under the lock, then poisons and frees outside it. The runtime-operation gate covers the complete operation so forced cleanup cannot overtake unfinished publication. Module allocation counters remain atomic and exported ABI layouts are unchanged.

The loader retains a locked read-only package stream for every module with overlays. Each process lazily allocates one `MALLOC_CAP_EXEC | MALLOC_CAP_32BIT` window sized to the largest overlay in its exact dependency closure. A transition reads into an ordinary DRAM staging buffer, hashes incrementally, and copies, clears, reads, and relocates the executable window only through aligned 32-bit accesses. Logical return placement is tracked separately from the payload physically retained in the window, so returning to resident code does not force the next call to reload the same overlay. A nested cross-overlay return still reloads the caller before execution resumes. Any failure after the previous payload may have been overwritten terminates that process entirely from resident code. This intentionally retains a one-window peak executable-memory bound and does not cache multiple payloads, prefetch, compress, or pin them.

Draft 0.46 layers storage without exposing ESP-IDF handles to C~ or to managed modules. `System.Storage.BlockDevice` owns a reference-counted native device or slice. The `ctilde_storage` component adapts ESP-IDF's SDMMC block-device layer to FatFs disk I/O, registers each volume with VFS, and owns SPI2 until all card, slice, and mount references are released. Raw I/O is rejected while a device is mounted. Formatting is a separate explicit operation; mounting never mutates media layout.

The removable-card monitor is the owner of card detection, partition slices, mounts, and rollback. Before an unavailable generation is unmounted, it calls the managed runtime's prefix invalidation hook. The runtime closes matching process-owned file and directory handles and resets matching current directories. Its ABI 19 service dispatcher translates generated `System.IO` adapters into the firmware VFS while keeping `FILE*`, directory streams, BDL devices, and FatFs objects private. An absent card remains retryable, while a configuration, partition, or mount fault is latched until the caller explicitly requests a remount; this avoids a permanent fast retry loop for faults that cannot heal through reinsertion alone. ManagedShell adds module-name precedence and one example-local, versioned storage-control accessor. The resident platform serializes privileged monitor, format, and MBR operations without exporting device or ESP-IDF handles; `sd.ctm` owns command parsing, validation, policy, and presentation. `/sd/modules` is probed before `/storage/modules`, and any existing SD candidate is authoritative even when its image is invalid.

ManagedShell diagnostics use a separate example-local boundary rather than Runtime ABI 22. `memory.ctm` and `taskmgr.ctm` contain their own reporter code, strings, stable insertion sorting, and `[Extern]` entry points through `managedModule.nativeSources`. The resident firmware exports one size- and version-checked diagnostics accessor. Protocol version 2 uses fixed-width, example-owned heap and task records plus wrapper functions instead of exposing ESP-IDF or FreeRTOS structures to module C. The table stores no module callback, so unloading either diagnostics module cannot leave a firmware pointer into unmapped module code.

Draft 0.49 applies the same boundary pattern to the shared shell and SSH. Firmware supervises `shell.ctm --uart`; redirected SSH sessions start that same application as `--ssh` or `--exec`. Reference-counted console endpoints choose UART or independent 8 KiB pipes, serialize writers, and give shared stdin to one foreground process at a time. The process registry records parents, owns source-created FreeRTOS tasks, and reclaims descendants, endpoints, and resident-accessor resources before arenas and module code. The shell accessor copies process/module records and forwards foreground, descendant-cleanup, LED, and UART-activity operations without retaining a module callback. Draft 0.50 keeps the shell entry/editor loop boundary resident and divides editor, parser/dispatch, filesystem, help, and process administration into bounded overlay groups.

The SSH host accessor exposes generation-checked opaque socket and cryptographic tokens, not descriptors or MbedTLS pointers. Resident cleanup callbacks own the underlying resources, so forced process cleanup never calls unloadable module code. Protocol framing, key exchange, authentication, channels, shell routing, and SFTP live in `system.ssh.ctm`; Draft 0.50 partitions service, transport, channels, configuration, key exchange, authentication, and three SFTP groups while the active connection state machine and stable entry stubs remain resident. `sshd.ctm` validates its invocation and calls the library's default-options factory and service loop. This is an accounting and unload boundary, not a security boundary: managed modules remain trusted native code.

Draft 0.42 still forbids direct managed strings at native boundaries. Explicit `System.Text.Utf8` conversion validates and copies bounded native bytes into an owned `ct_string`, while `NativeUtf8String` remains the zero-allocation borrowed C-string input path. The conversion helpers operate on pointer or native-buffer storage and never infer native ownership or deallocation.
...
- Atomic, non-moving ARC with immortal static strings, inline compiler-internal common paths, strict attachment-checked public ownership functions, and a per-thread allocation-free iterative release worklist.
...
`ct_runtime_initialize` attaches the primary thread, creates fault singletons, validates the selected Runtime ABI, initializes the module descriptor, and publishes ready. `ct_runtime_shutdown` rejects attached secondary threads, finalizes managed static fields in reverse order, drains ARC work, and detaches the primary thread. Hosted output stores the state pointer in C thread-local storage. Ordinary ESP-IDF firmware stores it in a reserved FreeRTOS task-local-storage slot with a task-deletion callback. Source-created workers use the same attachment contract, except that Runtime ABI 22 rejects attachment to an overlay-enabled dependency closure. Compiler-generated ARC common paths rely on that established attachment and consult thread state only on final release; public ownership calls check attachment before every operation. Object allocation initializes an unassigned atomic identity and default hashing assigns it lazily. ARC atomics protect lifetime only; ordinary managed fields and slots require `Atomic<T>`, volatile publication, or mutex/thread synchronization when shared.

The managed-module host replaces the single-program assumption with registry-owned module descriptors and process instances. The loader-created managed main task has an execution context containing its process, a configurable managed-call stack independent of the dependency limit, logical overlay return placement, physical window ownership and generation, runtime-operation depth, and runtime thread state. Source-created workers attach through the creating process context, receive independent runtime thread state, increment its task count, and are joined on normal completion or deleted before forced reclamation. Normal completion finalizes process-local module statics in reverse dependency order, drains tracked allocations and messages, closes retained overlay streams, releases the application and newly unused dependencies in reverse order, then signals completion. Draft 0.49 uses reference-counted UART/pipe console endpoints and reclaimable monotonically assigned process identities. Forced termination is submitted to a dedicated control task, which closes an atomic stop-bit/operation-count gate before deleting tasks at a stable runtime boundary and invokes only resident native-resource cleanup callbacks. Physical overlay-source loss marks matching streams unavailable and forces affected processes to resident cleanup without running user finalizers.
