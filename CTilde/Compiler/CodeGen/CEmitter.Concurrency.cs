...
internal sealed partial class CEmitter
{
    ...
    private void EmitManagedThreadSupport(CWriter writer, TypeSymbol thread)
    {
        ...
        writer.WriteLine("#elif defined(ESP_PLATFORM)");
        writer.WriteLine($"    uint32_t stack_bytes = thread->{stack}; uint32_t stack_words = stack_bytes == 0u ? (uint32_t)configMINIMAL_STACK_SIZE * 4u : (stack_bytes + 3u) & ~UINT32_C(3); UBaseType_t native_priority = (UBaseType_t)(tskIDLE_PRIORITY + 1u + (unsigned)thread->{priority});");
        writer.WriteLine("    payload->Done = xSemaphoreCreateBinary(); if (payload->Done != NULL) { ct_atomic_store_release(&payload->Ready, 1u); created = xTaskCreate(ct_managed_thread_worker, \"C~ worker\", stack_words / 4u, thread, native_priority, &payload->Handle) == pdPASS; if (!created) ct_atomic_store_relaxed(&payload->Ready, 0u); }");
        writer.WriteLine("#else");
