...
The expected exit code is `0`. The fixed arena is an example allocator, not a production allocator. Draft 0.43 also permits this target to use the ordinary console, time, math, file, directory, stream, thread, and mutex APIs when the project supplies the corresponding typed `RuntimeImpl` service group; see the normative role table in [LANGUAGE.md](../../LANGUAGE.md). This minimal kernel supplies only allocate, free, and panic because those are the only roles it reaches. A complete OS-service provider example remains listed explicitly in [TODO.md](../../TODO.md#example-coverage).
