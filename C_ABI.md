...
Symbol-map schema 1 additively records native entry/export names, task stack bytes, and declared stack-usage bytes. GCC stack reporting uses `-fstack-usage -fcallgraph-info=su`; LTO consumes final `.ltrans` output. Stack-report schema 1 records target/toolchain data, frame qualifiers, native roots, lower and complete bounds, longest paths, contracts, task headroom, and explicit unknown boundaries. These reports and attributes do not change runtime ABI 17 or public-header signatures.
