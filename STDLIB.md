...
`Send`, `Receive`, and timed `TryReceive` use copied `byte[]` messages. A process receives only from its own mailbox; `Receive` throws `InvalidOperationException` when called through a handle other than `Current`, while `TryReceive` returns false. Managed object references never cross a process boundary. Processes loaded from the same module have separate mutable statics, allocation accounting, arguments, cancellation state, and mailboxes; code and immutable module metadata are shared. A completed process has released its application module before `HasExited` becomes true. Managed modules are trusted native code without memory protection. Forced termination currently stabilizes and deletes the main process task, then reaps its tracked allocation arena, mutable-static instances, mailbox, and application module. Runtime faults raised through the shared module host mark the process failed, but an arbitrary unhandled managed exception does not yet have a complete top-level process boundary. Forced cleanup cannot interrupt a blocking console read or a normal finalizer after the main task has handed cleanup ownership over. Source-created child-task ownership, managed dependency APIs, and the native-resource ledger remain incomplete. Behavior is undefined if unsafe code has leaked pointers, locks, callbacks, interrupt registrations, or untracked native resources.
