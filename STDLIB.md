...
This document is the canonical standard-library reference for C~ Draft 0.50 and Runtime ABI 22. Runtime ABI 22 retains the managed-process filesystem, storage, and redirected-stream APIs and cleanup-safe managed-call transitions for process-local code overlays. Draft 0.50 also includes incremental text reading, scalable stable sorting, and correctness fixes. Managed Module ABI 3 and debug metadata version 3 remain current.
...
The [CollectionsAndGeometry example](examples/CollectionsAndGeometry/README.md) composes `Vec2` and `Vec4` values with both matrix types and a quaternion in one deterministic hosted run.
...
`System.Diagnostics.Process` is available to ESP-IDF firmware that links the shared managed-module runtime. `Start(path, args)` loads an application module, creates an independent process instance, copies its arguments, and starts its entry point on a FreeRTOS task. A bare filename searches `/sd/modules` first and `/storage/modules` second; only an absent SD entry selects the fallback. An absolute path must name a direct child of exactly one of those roots. Nested paths are rejected because the current Espressif loader identifies modules by basename. `Current` returns the current managed process from module code; it is `null` in ordinary firmware execution. `Id`, `State`, `HasExited`, and `ExitCode` inspect a process. `Wait()` and timed `Wait(uint, out int)` wait for completion. `Cancel()` requests cooperative cancellation, exposed inside the current process as `Process.IsCancellationRequested`. `Terminate(uint)` and `Terminate(TimeSpan)` submit cancellation and forced deletion to the firmware termination-control task. A grace value of `uint.MaxValue`, including the exact equivalent `TimeSpan`, requests an unbounded cooperative wait and never reaches forced deletion. Callers do not own the target task's deletion sequence.

`Send`, `Receive`, and timed `TryReceive` use copied `byte[]` messages. A process receives only from its own mailbox; `Receive` throws `InvalidOperationException` when called through a handle other than `Current`, while `TryReceive` returns false. Managed object references never cross a process boundary. Processes loaded from the same module have separate mutable statics, allocation accounting, arguments, cancellation state, mailboxes, parent relationships, and console endpoints; code and immutable module metadata are shared. A completed process has released its application module before `HasExited` becomes true. Managed modules are trusted native code without memory protection. Nonredirected streams inherit reference-counted parent endpoints, while each requested redirect creates a dedicated 8 KiB pipe. Shared input is readable only by its foreground process. Source-created tasks inherit process context and are joined or forcibly deleted with their process. An uncaught application-main exception writes a bounded diagnostic to stderr and exits with `-2`. Forced cleanup wakes endpoints, closes process-owned files and resident-ledger resources, deletes remaining tasks, and then reclaims the managed arena and module graph. A normal finalizer which has already taken cleanup ownership cannot yet be promoted back to forced cleanup. Behavior is undefined if unsafe code has leaked pointers, locks, callbacks, interrupt registrations, or untracked native resources.
...
`Stream` defines capabilities, length and position, ranged reads/writes, byte operations, `ReadExactly`, seek, truncation, flush, copy, and idempotent disposal. `FileStream` owns one private runtime handle and throws `ObjectDisposedException` after disposal. Stream instances are not thread-safe. `StreamReader` uses a 4096-byte read buffer and accumulates only the requested line or remaining text. It strips one leading UTF-8 BOM, including a BOM split across reads, and handles LF and CRLF. `ReadLine` returns after a complete line is available without waiting for EOF. Strict UTF-8 validation applies to the consumed result; invalid later input is reported when consumed. Non-seekable streams and fragmented multibyte input are supported. `StreamWriter` uses a 4096-byte output buffer, deterministic LF endings, optional `leaveOpen`, and writes one BOM only when `UTF8WithBom` begins an empty seekable stream.
...
`Esp.Idf.Storage.SdSpiConfiguration.TCan485` selects SPI2 with MISO 2, MOSI 15, SCLK 14, CS 13, and a 20 MHz clock. `SdSpiCard.Open` owns the card, bus, and root block device until explicit disposal. `RemovableSdCardMonitor` accepts whole-card or primary-partition FAT mappings, reports `NotPresent`, `Mounting`, `Mounted`, `Removing`, or `Faulted`, probes status every 500 ms, retries an absent card every second, invalidates mounted generations before unmount, and remounts after reinsertion. Configuration, partition, and mount failures stay `Faulted` until `Remount` explicitly requests another attempt. Current support is limited to the classic T-CAN485 SDSPI wiring.

Selecting any `System.Storage` type in a non-freestanding compilation automatically includes the internal `NativeUtf8String` and common I/O declarations used by the storage implementation. User code does not need to mention those dependencies. The current native block-device, FAT, and removable-card implementation is ESP-IDF-specific; freestanding compilation does not expose this surface.
...
The callback types are `Predicate<T>`, `Equality<T>`, `Ordering<T>`, `Mapper<TInput,TOutput>`, `Folder<TState,TValue>`, and `Visitor<T>`. `System.Collections.ArrayAlgorithms` provides `Copy`, `Contains`, `IndexOf`, `FindIndex`, `Any`, `All`, `Count`, `ForEach`, `Map`, `Filter`, `Fold`, `Reversed`, and `Sorted`. Traversal operations visit input from left to right. `Any` and `All` short-circuit, and `Filter` invokes its predicate exactly once per input. `Sorted` preserves equal-key order but does not promise a comparer invocation sequence. It uses insertion sort for runs of at most 32 elements, followed by iterative stable merge passes with one scratch array. Worst-case comparison growth is O(n log n). Allocating operations return new arrays and never mutate the source. Callback exceptions propagate with ARC cleanup. Equality and ordering are explicit; there is no boxing fallback.
...
The focused [CollectionsAndGeometry example](examples/CollectionsAndGeometry/README.md) exercises each generic value type and every mutable collection type, including a caught version-mismatch failure after mutating an actively enumerated list.
