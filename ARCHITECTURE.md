...
Draft 0.41 retains the single typed-IR and C backend while adding a native-build profile layer after deterministic C emission and a compiler-recognized standard-library surface for the built-in UTF-8 string type. Project and CLI settings resolve before native compilation; exact compiler flags, object-cache keys, and hosted PGO identities share the resolved optimization, CPU, floating-point, compiler, generated-source, and LTO inputs. The target-specific Cosmopolitan design remains in [COSMOPOLITAN.md](COSMOPOLITAN.md).
...
Draft 0.41 still forbids direct managed strings at native boundaries. Explicit `System.Text.Utf8` conversion validates and copies bounded native bytes into an owned `ct_string`, while `NativeUtf8String` remains the zero-allocation borrowed C-string input path. The conversion helpers operate on pointer or native-buffer storage and never infer native ownership or deallocation.
