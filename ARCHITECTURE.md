...
- Debug-enabled emission adds coalesced project-relative `#line` mappings, resets generated runtime regions to `<ctilde-generated>`, and writes a separate deterministic version-1 debug map. The map relates retained C functions, executable spans, locals, durable `ct_lp_*` storage, types, fields, and stable exception hooks without embedding toolchain paths.
...
The VS Code client starts `dotnet CTilde.LanguageServer.dll`, synchronizes `.ct` and `ctilde.json` changes, maps embedded declarations to the read-only `ctilde-stdlib:` scheme, and contributes the project schema. It also discovers manifests and supplies Check, Build, Debug, and Attach workflows that launch the short-lived CLI. A standalone Node DAP process drives GDB through token-correlated MI records and uses the debug map to translate frames, source locations, locals, strings, arrays, and exception-hook stops. ESP UART sessions use the ESP-IDF Python environment as a persistent serial-to-GDB pipe, so interrupt and remote-protocol bytes remain on one open port. MSVC delegates to `cppvsdbg`. The packaged extension includes framework-dependent server and compiler assemblies plus the debug adapter; the .NET 10 runtime remains an external requirement for the bundled copies. Window-scoped overrides can select external built server or compiler artifacts. Only the long-running development server needs shadow copying and restart coordination.
