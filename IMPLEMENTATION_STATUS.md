...
C~ Draft 0.45 has one compiler path:
...
The compiler library, CLI, and conformance runner target .NET 10. Draft 0.45 introduces Runtime ABI 18 and Managed Module ABI 1 for trusted ESP-IDF ELF applications that share one firmware runtime. The implemented application-module slice includes deterministic `.ctmeta.json`, a fixed `CTMOD` ELF manifest, hidden module-local definitions, runtime API and descriptor accessors, per-process mutable-static schemas, FreeRTOS-backed process startup, copied arguments and messages, cooperative cancellation state, allocation accounting, and immediate unload after process cleanup. Managed libraries, canonical cross-module type registration, shared dictionary-driven generics, complete resource-ledger termination, and process-level exception isolation remain in progress. Draft 0.44 remains the dependency-oriented modular-C and GCC stack-analysis revision.
...
The connected-board acceptance then completed 16 warmup and 100 measured load/run/unload cycles without a crash or failed process. Current free heap was exactly 167,000 bytes before and after the measured cycles, and the module registry reported zero loaded modules at both measurements. Completed process records reported exit code 1, zero managed heap, zero tasks, and no retained module. The minimum-ever heap moved from 122,908 to 122,868 bytes, while current free heap remained stable. The serial monitor was closed after the run, leaving COM4 free. This is application-module lifecycle evidence, not acceptance of the still-outstanding shared-generics, managed-library, forced-termination, and unsafe-resource cases.
...
| Runtime ABI 18 lifecycle | Implemented | Ordinary program initialization/shutdown, module descriptor, reverse static finalization, native attachment, source-created workers, thread gates, panic callback tests, hosted acceptance, typed freestanding/ESP-IDF service-provider boundary, Draft 0.44 connected-board acceptance, and the Draft 0.45 managed-application lifecycle slice |
| Managed Module ABI 1 application slice | In progress | Deterministic metadata and ELF manifest, hidden module-local storage, runtime binding, per-process statics/heaps/mailboxes, FreeRTOS entry task, immediate unload, and stable 100-cycle ESP32 acceptance are implemented; shared generics, managed-library imports, exception isolation, complete termination, and resource accounting remain outstanding |
...
The full examples in [examples/Features/Program.ct](examples/Features/Program.ct), [examples/ObjectModel/Program.ct](examples/ObjectModel/Program.ct), and [examples/Exceptions/Program.ct](examples/Exceptions/Program.ct) are part of the native and ABI checks. The Features project links the same checked-in native shim used by conformance testing through `hosted.nativeSources`.
...
On 2026-09-02, the Draft 0.45 Release solution build completed with zero warnings and errors and the complete conformance run passed all 250 registered cases. The focused `managed-module` cases additionally cover deterministic ABI artifacts, hidden ELF definitions, Runtime ABI console routing, stale metadata rejection, and the project manifest contract. The ManagedShell firmware and `Hello` application module built with zero warnings and errors, and the connected ESP32 completed the 100-cycle acceptance described above. Cross-toolchain, ESP32-C3, QEMU, editor packaging, and the incomplete managed-library/shared-generics cases were not rerun or claimed for this tranche.

Draft 0.45 keeps GCC or Clang in GNU C23 mode as the canonical freestanding release gate and the pinned Cosmopolitan 4.0.2 wrapper for x64 APE acceptance; stack-specific gates use GCC only. MSVC latest-C mode remains an independent hosted compatibility check and rejects inline-assembly, assembly-function, and stack-report native builds. ESP-IDF interrupt and stack-instrumented output target Xtensa and RISC-V GNU toolchains. Unity and modular layouts consume the same optimized typed-IR program and must agree under every supported target/toolchain combination. Native-import acceptance is hosted Windows/Linux only; automatic scalar-geometry SIMD acceptance remains hosted x64 only. Managed ELF modules are ESP-IDF-only and presently use Espressif's ELF loader on 32-bit Xtensa. Historical Draft 0.44 stack analysis/modular C, Draft 0.43 runtime services, Draft 0.42 parsing/I/O, Draft 0.41 string/native optimization, Draft 0.40 foundations, Draft 0.39 native imports, Draft 0.38 SIMD, Draft 0.24 Cosmopolitan, Draft 0.23 interrupt, Draft 0.22 effects, Draft 0.21 freestanding, and Draft 0.20 connected-board baselines remain recorded above.
