...
- `EffectRegistry` records allocation, exception, blocking, runtime-use, and call operations during analysis-only lowering. `EffectAnalyzer` computes recursive effects to a deterministic fixed point, applies trusted and inherited contracts, and reports full witnesses for `[NoAlloc]`, `[NoThrow]`, `[NoBlock]`, and `[NoRuntime]`. Freestanding heap inference and runtime-hook bootstrap validation query the same result.
