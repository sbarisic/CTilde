    ...
    public static void RegisterPart10(ConformanceSuite suite)
    {
        ...
        suite.Run("draft 0.14 deterministic C~ debug information", () =>
        {
            ...
            try
            {
                ...
                Assert(debugMap.Contains("\"version\": 3", StringComparison.Ordinal) && debugMap.Contains($"\"runtimeAbi\": {CompilerContract.RuntimeAbiVersion}", StringComparison.Ordinal) && debugMap.Contains("\"instrumented\": false", StringComparison.Ordinal), "Source debug metadata did not use the v3 non-instrumented contract.");
