...
`ctilde.json` defines deterministic source globs, exclusions, and target settings. `kind` defaults to `application`; `standard-library` projects validate physical standard-library declarations without an application entry point or emitted output. Hosted applications can add contained, unique `.c` files through `hosted.nativeSources`. The CLI and language server share the loader. Files without a manifest are analyzed as standalone hosted programs; files outside a manifest source set retain that manifest's target but do not join its compilation.
