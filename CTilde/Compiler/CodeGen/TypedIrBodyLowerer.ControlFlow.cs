...
internal sealed partial class TypedIrBodyLowerer
{
    ...
    private static int CountDefer(StatementSyntax statement) => statement switch
    {
        ...
        StaticIfStatementSyntax @if => CountDefer(@if.Then) + (@if.Else is null ? 0 : CountDefer(@if.Else)),
    ...
    private static int CountTry(StatementSyntax statement) => statement switch
    {
        ...
        StaticIfStatementSyntax @if => CountTry(@if.Then) + (@if.Else is null ? 0 : CountTry(@if.Else)),
    ...
    private static bool ContainsThrow(StatementSyntax statement) => statement switch
    {
        ...
        StaticIfStatementSyntax @if => ContainsThrow(@if.Then) || @if.Else is not null && ContainsThrow(@if.Else),
