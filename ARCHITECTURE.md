...
Draft 0.46 retains the single typed-IR and C backend while adding a portable block-device/FAT mount surface, an ESP-IDF SDSPI/BDL/FatFs adapter, removable-media generations, and Runtime ABI 19 process-owned VFS services. Ordinary hosted, Cosmopolitan, freestanding, and ESP-IDF firmware artifacts retain their existing startup model. Draft 0.45 remains the managed-module artifact revision; the target-specific Cosmopolitan design remains in [COSMOPOLITAN.md](COSMOPOLITAN.md).
...
`Examples.sln` is a discovery surface, not a cross-target build graph. Its 20 C~ projects are grouped as language/hosted programs, systems targets, managed modules, and T-CAN variants; none has a `Build.0` mapping. The managed group contains separate firmware, Hello application, memory-tool, and task-manager projects. The [example catalog](examples/README.md) records each project's toolchain or hardware prerequisite. Focused hosted examples execute common features without optional hardware, while target examples retain their own explicit runners.
...
Draft 0.45 adds a separate managed edge for ESP-IDF only. Firmware owns the module and process registries, runtime table, task-local process context, allocation ledger, and ELF loader. A `.ctm` contains no `app_main` or private copy of that firmware host; Draft 0.46 binds it to Runtime ABI 19, gives it process-specific mutable-static and current-directory state, and keeps it mapped while its process or a tracked dependent, active call, managed allocation, or type registration can refer to it. Type descriptors already carry stable 128-bit fingerprints, generated casts compare those fingerprints, and the runtime registry validates, reuses, and removes descriptor registrations around module lifetime. Generated object, interface, and delegate metadata still uses module-local descriptor addresses instead of adopting the canonical descriptor returned by registration, and runtime-sized `ct_type_ops` dictionaries do not yet exist. The loader already preflights, initializes, retains, and releases an exact acyclic dependency graph, but the compiler does not yet consume provider declarations or generate callable managed imports. Reachable compiler-generated language-runtime and standard-library helpers also remain inside each module. This architecture is independent of hosted `[NativeImport]`, whose types and ownership stop at the native C ABI.
...
`ct_runtime_initialize` attaches the primary thread, creates fault singletons, validates Runtime ABI 19, initializes the module descriptor, and publishes ready. `ct_runtime_shutdown` rejects attached secondary threads, finalizes managed static fields in reverse order, drains ARC work, and detaches the primary thread. Hosted output stores the state pointer in C thread-local storage. Ordinary ESP-IDF firmware stores it in a reserved FreeRTOS task-local-storage slot with a task-deletion callback. Source-created workers use the same attachment contract. Compiler-generated ARC common paths rely on that established attachment and consult thread state only on final release; public ownership calls check attachment before every operation. Object allocation initializes an unassigned atomic identity and default hashing assigns it lazily. ARC atomics protect lifetime only; ordinary managed fields and slots require `Atomic<T>`, volatile publication, or mutex/thread synchronization when shared.

The managed-module host replaces the single-program assumption with registry-owned module descriptors and process instances. The loader-created managed main task has an execution context containing its process, active module call stack, runtime-operation depth, and runtime thread state. Normal completion finalizes process-local module statics in reverse dependency order, drains tracked allocations and messages, releases the application and newly unused dependencies in reverse order, then signals completion. A one-way published process identifier prevents partially initialized handles from escaping. Forced termination is submitted to a dedicated control task, which closes an atomic stop-bit/operation-count gate before deleting the main task at a stable runtime boundary. The FreeRTOS TLS deletion callback abandons completed call/operation records and queues blocking cleanup to a separate reaper. Callable managed-library imports, blocking console input, normal finalization already in progress, process inheritance, and coordinated cleanup for source-created child tasks remain outstanding.
