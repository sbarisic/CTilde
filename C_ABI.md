...
This document defines the generated C contract for C~ draft 0.46, Runtime ABI 19, and Managed Module ABI 1. Draft 0.46 adds process-owned VFS services and removable-storage invalidation; ordinary native exports retain the Draft 0.45 layouts unless this document states otherwise.

Draft 0.46 uses Runtime ABI 19 and debug metadata version 3. CPU, floating-point, LTO, PGO, and stack instrumentation affect native code generation only. SIMD values cannot cross native boundaries; matrices and quaternions use the existing natural-layout aggregate rules. Ordinary effect and stack contracts do not change native signatures, public headers, name mangling, or ABI identity. An `[Interrupt]` export intentionally emits the requested native symbol directly with the fixed `void(void*)` ABI and records that fact in the header signature. ABI 19 output is not ABI-compatible with ABI 18 or older generated modules. `[Export]`, function/data `[Extern]`, linker symbols, and documented runtime ABI names remain stable native names; all other generated names are implementation artifacts. `[NativeImport]`, introduced in Draft 0.39, uses private runtime-resolved slots and does not add a public ABI name. `[Used]` guarantees final-image retention on supported ELF and COFF toolchains. Open generics, interface references, SIMD values, `Atomic<T>`, `Thread`, and `Mutex` cannot cross an ordinary native boundary.

Debug information is additive and does not change Runtime ABI 19. Source-debug output may contain `#line` directives and private non-inlined exception hooks. Instrumented debug-preparation output additionally contains logical probes, a private debugger control block, per-thread debug frames, and optional private allocation-registry or guarded-allocation prefixes. These layouts exist only inside the matching instrumented image, are absent from ordinary output, and are not exported native contracts. Debug-map and target-descriptor version 3 include aggregate layout metadata alongside closed-generic names, interface views, atomic storage, runtime thread IDs, and Thread/Mutex presentation.
...
`Compilation.EmitSymbolMap`, CLI `--symbol-map`, and modular bundles emit version 1 JSON sorted by compact name. Each entry includes the compact name, full canonical identity, kind, signature/result type, and source location. The map declares Runtime ABI 19.
...
Managed modules are ELF32 `ET_DYN` images for the resolved ESP-IDF architecture. They contain a `.ctilde.manifest` record whose fixed-width header identifies Runtime ABI 19, Managed Module ABI 1, architecture, kind, canonical name and version, build identity, API hash, task stack, heap limit, and exact dependencies. Names occupy a 64-byte array and accept at most 63 ASCII bytes; versions occupy a 32-byte array and accept at most 31 ASCII bytes. The final byte remains available for the required NUL terminator. The compiler applies these limits to project, reference, and dependency identities before emitting C, and the loader validates the record from bytes before relocation. Module files are direct children of `/sd/modules` or `/storage/modules`; bare names search SD first and use LittleFS only when the SD entry is absent. The flat namespace matches Espressif's basename-keyed dynamic-module registry and prevents preflight/relocation path aliasing.

The only callable dynamic ABI entries are `ct_managed_module_descriptor()` and `ct_managed_module_bind_runtime(const ct_runtime_api_v19*)`. The preflight manifest is retained as a dynamic object solely so Espressif `project_so(...)` section garbage collection cannot discard it. All other generated definitions have hidden visibility and resolve through module-relative relocations. A conforming module must have no `R_XTENSA_GLOB_DAT` or equivalent relocation for module-owned objects. The loader may reject any undefined symbol not present in its explicit native allowlist.

`ct_managed_module_descriptor_v1` identifies the module and supplies per-process static-state size/alignment, initialize/finalize callbacks, application entry, argument construction, byte-message construction, and resource limits. Function and string pointers in the descriptor are relocated module addresses. The firmware keeps the module mapped while any registered runtime reference can reach those addresses.

`ct_runtime_api_v19` begins with `Size` and `AbiVersion`, followed by allocation/free/final-release, exception/fault, canonical type registration, current process/module/thread state, cancellation, managed call pinning, and generic runtime-service slots. A module checks the size and ABI before storing the borrowed table pointer. Console services use service ordinals 16, 17, and 18 with `ct_runtime_console_transfer_v19`; thread attach/detach use ordinals 1 and 2. Runtime ABI 19 adds file service ordinals 32 through 39 and path, metadata, directory, and current-directory ordinals 48 through 58. Their sized payloads contain borrowed UTF-8 byte spans, 64-bit offsets and lengths, process-owned opaque handles, stable status/native-error results, and caller-owned output buffers.

Every generated `ct_type_descriptor` carries a stable 128-bit fingerprint in addition to its module-local address. Casts, interface matching, boxing, and unboxing accept equal fingerprints. `RegisterType` returns an existing canonical descriptor when the fingerprint, name, size, alignment, and value/reference kind agree, rejects incompatible collisions, and otherwise registers the provider descriptor until that module is removed. Managed Module ABI 1 generated initialization currently registers each descriptor but continues to embed its local descriptor addresses in object and dispatch metadata. Treating the returned canonical address as authoritative, validating the complete layout and operation contract, and adding runtime-sized `ct_type_ops` remain future ABI work.
...
Symbol-map schema 1 additively records native entry/export names, task stack bytes, and declared stack-usage bytes. GCC stack reporting uses `-fstack-usage -fcallgraph-info=su`; LTO consumes final `.ltrans` output. Stack-report schema 1 records target/toolchain data, frame qualifiers, native roots, lower and complete bounds, longest paths, contracts, task headroom, and explicit unknown boundaries. These reports and attributes do not change Runtime ABI 19 or public-header signatures.
