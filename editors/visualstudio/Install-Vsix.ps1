...
function Resolve-VsixFile {
    ...
    if ([string]::IsNullOrWhiteSpace($Path) -or -not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw 'VSIX not found. Pass -VsixPath or run Build-Vsix.ps1 first.'
    }
...
foreach ($instance in $instances) {
    ...
    $logPath = Join-Path ([System.IO.Path]::GetFullPath($LogDirectory)) "install-$($instance.instanceId).log"
    $arguments = @('/quiet', "/instanceIds:$($instance.instanceId)", "/logFile:$logPath")
    if ($ShutdownProcesses) { $arguments += '/shutdownprocesses' }
    $arguments += $resolvedVsix

    if (-not $PSCmdlet.ShouldProcess("$($instance.displayName) [$($instance.instanceId)]", "Install '$resolvedVsix' silently")) {
        continue
    }
    ...
    if ($exitCode -ne 0) {
        throw "VSIX installation failed for $($instance.displayName) with exit code $exitCode. Log: $logPath"
    }

    Write-Host "Installed '$([System.IO.Path]::GetFileName($resolvedVsix))' for $($instance.displayName). Log: $logPath"
}
